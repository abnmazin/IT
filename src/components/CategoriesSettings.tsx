    ...
    <div className="p-3 sm:p-6 space-y-4">
      ...
      <div className="hidden sm:block bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {categories.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-400">لا توجد فئات.</div>
          ) : (
            categories.map((cat) => (
              <div key={cat.id} className="grid grid-cols-[1fr_1fr_auto_auto_auto] gap-4 items-center px-5 py-3 hover:bg-slate-50 transition-colors">
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${cat.serialTracked ? "bg-sky-50 text-sky-700" : "bg-slate-100 text-slate-500"}`}>
                  {cat.serialTracked ? "تسلسلي" : "كمي"}
                </span>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${cat.consumable ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>
                  {cat.consumable ? "استهلاكي" : "يعاد"}
                </span>
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(cat)} className="w-10 h-10 rounded-lg flex items-center justify-center hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors" title="تعديل">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(cat)} className="w-10 h-10 rounded-lg flex items-center justify-center hover:bg-red-50 text-slate-400 hover:text-red-600 transition-colors" title="حذف">
              ...
              <div className="flex items-start justify-between mb-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-mono text-slate-700 truncate">{cat.key}</span>
                  </div>
                  <p className="text-sm font-semibold text-slate-900 mt-1 truncate">{cat.label}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0 mr-2">
                  <button onClick={() => openEdit(cat)} className="w-10 h-10 rounded-lg flex items-center justify-center bg-slate-50 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(cat)} className="w-10 h-10 rounded-lg flex items-center justify-center bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${cat.serialTracked ? "bg-sky-50 text-sky-700" : "bg-slate-100 text-slate-500"}`}>
                  {cat.serialTracked ? "تسلسلي" : "كمي"}
                </span>
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${cat.consumable ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>
                  {cat.consumable ? "استهلاكي" : "يعاد"}
                </span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowModal(false)}>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">المفتاح (EN) *</label>
                <input
                  type="text"
                  value={key}
                  onChange={(e) => setKey(e.target.value)}
                  placeholder="مثال: Laptop, Mouse..."
                  className="w-full h-11 px-3 rounded-lg border border-slate-200 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500/30 focus:border-sky-400 font-mono"
                  autoFocus
                />
                <p className="text-xs text-slate-400 mt-1">اسم الفئة بالإنجليزية (يُستخدم كمعرف داخلي)</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">الاسم (AR) *</label>
                <input
                  type="text"
                  value={label}
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                ...
                >
                  {serialTracked ? <ToggleRight className="w-12 h-12 text-sky-500" /> : <ToggleLeft className="w-12 h-12" />}
                </button>
              </div>
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div className="min-w-0 flex-1 mr-3">
                ...
                <button
                  ...
                  className="text-slate-500 shrink-0"
                >
                  {consumable ? <ToggleRight className="w-12 h-12 text-amber-500" /> : <ToggleLeft className="w-12 h-12" />}
                </button>
              </div>
